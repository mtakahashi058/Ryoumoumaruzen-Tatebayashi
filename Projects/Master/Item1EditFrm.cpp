...
#pragma link "CEdit"
#pragma resource "*.dfm"
TItem1EditForm *Item1EditForm;
//---------------------------------------------------------------------------
__fastcall TItem1EditForm::TItem1EditForm(TComponent* Owner)
	: TForm(Owner)
{
}
//---------------------------------------------------------------------------
void __fastcall TItem1EditForm::FormShow(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TItem1EditForm::FormClose(TObject *Sender,
	  TCloseAction &Action)
{
	if (is_edit_)
		ModalResult = mrOk;
...
//---------------------------------------------------------------------------
void __fastcall TItem1EditForm::FormKeyDown(TObject *Sender, WORD &Key,
	  TShiftState Shift)
{
	switch (Key){
	case VK_DOWN:
...
//---------------------------------------------------------------------------
int __fastcall TItem1EditForm::ShowModalWithId(int id)
{
	id_ = id;
...
//---------------------------------------------------------------------------
void __fastcall TItem1EditForm::ClearForm()
{
      int code = GetNo();
      if (code != -1)
            CodeEdit->Text = code;
...
//---------------------------------------------------------------------------
void __fastcall TItem1EditForm::SetMode(TInputMode mode)
{
      if (mode == imNew){
            ModePanel->Caption       = "新規";
            ModePanel->Color         = TColor(NEW_CLR);
            //CodeEdit->ReadOnly       = false;
            //CodeEdit->Color          = clWindow;
...
//---------------------------------------------------------------------------
int __fastcall TItem1EditForm::GetNo()
{
    int code = 1;
    NsRecordSet set;
    DM->DBI.GetRecordSet("SELECT 項目1コード FROM M_項目1 WHERE 項目1コード > 0 ORDER BY 項目1コード", set);
    for (unsigned int i = 0; i < set.size(); i++){
        if (code != ToInt(set[i]["項目1コード"]))
            break;
        else
            code++;
    }
...
//---------------------------------------------------------------------------
bool __fastcall TItem1EditForm::CheckEdit()
{
	if (CodeEdit->Text.ToIntDef(0) == 0){
      ...
      sql = " SELECT"
            ...
            "     M_項目1"
            " WHERE"
            "     項目1コード = " + IntToStr(CodeEdit->Text.ToIntDef(0)) +
            " AND"
            "     項目1ID <> " + IntToStr(id_);
...
//---------------------------------------------------------------------------
bool __fastcall TItem1EditForm::PostRecord()
{
	NsFieldSet fields;

	fields << NsField("[項目1コード]",       CodeEdit->Text.ToIntDef(0))
		   << NsField("[項目1名称]",         NameEdit->Text)
		   << NsField("[項目1略称]",         ShortNameEdit->Text)
		   << NsField("[項目1カナ]",         KanaEdit->Text)
	...
	try{
		if (id_ == 0){
			...
			DM->DBI.Insert("M_項目1", fields);
		}else{
			DM->DBI.Update("M_項目1", "項目1ID", id_, fields);
		}
	}catch (Exception &e){
...
//---------------------------------------------------------------------------
void __fastcall TItem1EditForm::BrowseRecord()
{
	NsRecordSet set;
	AnsiString sql = "SELECT * FROM V_項目1 WHERE 項目1ID = " + IntToStr(id_);
	if (!DM->DBI.GetRecordSet(sql, set)){
	...
	id_                       = ToInt(set[0]["項目1ID"]);
	CodeEdit->Text            = ToString(set[0]["項目1コード"]);
	NameEdit->Text            = ToString(set[0]["項目1名称"]);
	ShortNameEdit->Text       = ToString(set[0]["項目1略称"]);
	KanaEdit->Text            = ToString(set[0]["項目1カナ"]);
...
//---------------------------------------------------------------------------
void __fastcall TItem1EditForm::AutoKanaMessage(tagMSG &Msg, bool &Handled)
{
...
//---------------------------------------------------------------------------
void __fastcall TItem1EditForm::Action1Execute(TObject *Sender)
{
      id_ = 0;
      SetMode(imNew);
      ClearForm();
...
//---------------------------------------------------------------------------
void __fastcall TItem1EditForm::Action3Execute(TObject *Sender)
{
    if (DM->GetMode(ModePanel) != imMod)
        return;

    if (!DM->DBI.IsExist("M_項目1", "項目1ID", id_)){
    ...
    if (ToInt(DM->DBI.Lookup("M_項目1", "項目1ID", id_, "使用区分")) != 0){
        ...
        NsFieldSet keys;
        keys << NsField("[項目1ID]", id_);
        try{
            DM->DBI.Delete("M_項目1", keys);
...
//---------------------------------------------------------------------------
void __fastcall TItem1EditForm::Action4Execute(TObject *Sender)
{
      if (ActiveControl == CodeEdit)
            SaibanButton->OnClick(NULL);
}
//---------------------------------------------------------------------------
void __fastcall TItem1EditForm::Action5Execute(TObject *Sender)
{
      if (!CheckEdit())
            return;
...
//---------------------------------------------------------------------------
void __fastcall TItem1EditForm::Action7Execute(TObject *Sender)
{
      if (DM->GetMode(ModePanel) == imMod && DM->M_Item1->Bof)
            return;
      ...
      id_ = DM->M_Item1->FieldByName("項目1ID")->AsInteger;
      BrowseRecord();
}
//---------------------------------------------------------------------------
void __fastcall TItem1EditForm::Action8Execute(TObject *Sender)
{
	if (DM->M_Item1->Eof)
		return;
...
//---------------------------------------------------------------------------
void __fastcall TItem1EditForm::Action12Execute(TObject *Sender)
{
	Close();
}
//---------------------------------------------------------------------------
void __fastcall TItem1EditForm::CodeEditExit(TObject *Sender)
{
      if (CodeEdit->Text == "")
            return;

      NsRecordSet set;
      if (!DM->DBI.GetRecordSet("SELECT 項目1ID FROM M_項目1 WHERE 項目1コード = " + CodeEdit->Text + " AND 項目1ID <> " + IntToStr(id_), set))
            return;
...
//---------------------------------------------------------------------------
void __fastcall TItem1EditForm::SaibanButtonClick(TObject *Sender)
{
      CodeEdit->Text = GetNo();
}
//---------------------------------------------------------------------------
void __fastcall TItem1EditForm::NameEditExit(TObject *Sender)
{
	if (DM->GetMode(ModePanel) == imMod)
		return;
...
//---------------------------------------------------------------------------
void __fastcall TItem1EditForm::CodeEditEnter(TObject *Sender)
{
	StatusBar->SimpleText = "  コードを入力して下さい（採番[F4]を押す事で、空き番検索を行います）";
}
//---------------------------------------------------------------------------
void __fastcall TItem1EditForm::NameEditEnter(TObject *Sender)
{
	StatusBar->SimpleText = "  名称を入力して下さい";
}
//---------------------------------------------------------------------------
void __fastcall TItem1EditForm::ShortNameEditEnter(TObject *Sender)
{
	StatusBar->SimpleText = "  必要な場合略称を変更して下さい";
}
//---------------------------------------------------------------------------
void __fastcall TItem1EditForm::KanaEditEnter(TObject *Sender)
{
	StatusBar->SimpleText = "  必要な場合カナを変更して下さい";
}
//---------------------------------------------------------------------------
void __fastcall TItem1EditForm::UseDivComboBoxEnter(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TItem1EditForm::UseDivComboBoxExit(TObject *Sender)
{
	Action4->Enabled = true;
}
//---------------------------------------------------------------------------
void __fastcall TItem1EditForm::UseDivComboBoxKeyDown(TObject *Sender,
...
//---------------------------------------------------------------------------
void __fastcall TItem1EditForm::CodePanelEnter(TObject *Sender)
{
	((TPanel *)Sender)->Color = TColor(0x00A8A8A8);
}
//---------------------------------------------------------------------------
void __fastcall TItem1EditForm::CodePanelExit(TObject *Sender)
{
