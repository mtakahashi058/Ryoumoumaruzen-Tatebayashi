...
{
    TitlePanel->Caption = DM->TrimSpace(ToString(DM->CFG["品目マスタ名称"])) + "を選択して下さい";

	SFM.AddLog("品目選択画面 車両ID/車番: " + ToString(SFM.Stashed("車両ID")) + "/" + ToString(SFM.car_rec["車番"]));
...
void __fastcall TMeigSelectForm::AllocateRecordToButtons(int top_rec_no, const AnsiString& kana1, const AnsiString& kana2)
{
	...
	for (int i = 0; i < 18; i++){
		...
		if (cur_rec_no >= record_count){
			...
			len = ToString(set[cur_rec_no]["品目略称"]).Length();
			if (len <= 20)
			...
			Buttons_[i]->Caption = ToString(set[cur_rec_no]["品目略称"]);
			Buttons_[i]->Tag     = ToInt(set[cur_rec_no]["品目ID"]);
...
{
	SFM.stash["品目ID"]     = dynamic_cast<TButton *>(Sender)->Tag;
	SFM.stash["品目略称"]   = dynamic_cast<TButton *>(Sender)->Caption;
