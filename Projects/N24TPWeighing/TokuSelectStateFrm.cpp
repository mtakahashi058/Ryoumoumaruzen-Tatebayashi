...
#pragma resource "*.dfm"
TTokuSelectForm *TokuSelectForm;
//---------------------------------------------------------------------------
__fastcall TTokuSelectForm::TTokuSelectForm(TComponent* Owner)
	: AbstractStateForm(Owner)
{
}
//---------------------------------------------------------------------------
void __fastcall TTokuSelectForm::FormCreate(TObject *Sender)
{
	SFM.AddStateForm(State::SelectToku, this);
...
//---------------------------------------------------------------------------
void __fastcall TTokuSelectForm::OnEnter()
{
    TitlePanel->Caption = DM->TrimSpace(ToString(DM->CFG["得意先マスタ名称"])) + "を選択して下さい";

	SFM.AddLog("得意先選択画面 車両ID/車番: " + ToString(SFM.Stashed("車両ID")) + "/" + ToString(SFM.car_rec["車番"]));
	...
	PlaySound(ToString(DM->Sounds["TokuSelect"]).c_str(), 0, SND_ASYNC | SND_FILENAME);
}
//---------------------------------------------------------------------------
void __fastcall TTokuSelectForm::OnEvent(Event::EventID event, void *param)
{
...
//---------------------------------------------------------------------------
void __fastcall TTokuSelectForm::OnExit()
{
}
//---------------------------------------------------------------------------
void __fastcall TTokuSelectForm::AllocateRecordToButtons(int top_rec_no, const AnsiString& kana1, const AnsiString& kana2)
{
	NsRecordSet set;
	DM->GetTokuSet(kana1, kana2, set);
	...
	for (int i = 0; i < 18; i++){
		...
		if (cur_rec_no >= record_count){
			...
			len = ToString(set[cur_rec_no]["得意先略称"]).Length();
			if (len <= 20)
			...
			Buttons_[i]->Caption = ToString(set[cur_rec_no]["得意先略称"]);
			Buttons_[i]->Tag     = ToInt(set[cur_rec_no]["得意先ID"]);
...
//---------------------------------------------------------------------------
void __fastcall TTokuSelectForm::CancelButtonClick(TObject *Sender)
{
	SFM.Transit(PrevState(SFM.GetCurrentStateID()));
}
//---------------------------------------------------------------------------
void __fastcall TTokuSelectForm::Item1ButtonClick(TObject *Sender)
{
	SFM.stash["得意先ID"]     = dynamic_cast<TButton *>(Sender)->Tag;
	SFM.stash["得意先略称"]   = dynamic_cast<TButton *>(Sender)->Caption;

	SFM.Transit(NextState(SFM.GetCurrentStateID()));
}
//---------------------------------------------------------------------------
void __fastcall TTokuSelectForm::ScrollUpButtonClick(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TTokuSelectForm::ScrollDownButtonClick(TObject *Sender)
{
	int lefttop = lefttop_rec_no_;
	lefttop += 3;
	AllocateRecordToButtons(lefttop);
...
//---------------------------------------------------------------------------
void __fastcall TTokuSelectForm::Kana1ButtonClick(TObject *Sender)
{
