...
#pragma resource "*.dfm"
TItem1SelectForm *Item1SelectForm;
//---------------------------------------------------------------------------
__fastcall TItem1SelectForm::TItem1SelectForm(TComponent* Owner)
	: AbstractStateForm(Owner)
{
}
//---------------------------------------------------------------------------
void __fastcall TItem1SelectForm::FormCreate(TObject *Sender)
{
	SFM.AddStateForm(State::SelectItem1, this);
...
//---------------------------------------------------------------------------
void __fastcall TItem1SelectForm::OnEnter()
{
    TitlePanel->Caption = DM->TrimSpace(ToString(DM->CFG["項目1マスタ名称"])) + "を選択して下さい";

	SFM.AddLog("項目1選択画面 車両ID/車番: " + ToString(SFM.Stashed("車両ID")) + "/" + ToString(SFM.car_rec["車番"]));
	...
	PlaySound(ToString(DM->Sounds["Item1Select"]).c_str(), 0, SND_ASYNC | SND_FILENAME);
}
//---------------------------------------------------------------------------
void __fastcall TItem1SelectForm::OnEvent(Event::EventID event, void *param)
{
...
//---------------------------------------------------------------------------
void __fastcall TItem1SelectForm::OnExit()
{
}
//---------------------------------------------------------------------------
void __fastcall TItem1SelectForm::AllocateRecordToButtons(int top_rec_no, const AnsiString& kana1, const AnsiString& kana2)
{
	NsRecordSet set;
	DM->GetItem1Set(kana1, kana2, set);
	...
	for (int i = 0; i < 18; i++){
		...
		if (cur_rec_no >= record_count){
			...
			Buttons_[i]->Tag         = 0;
			Buttons_[i]->HelpContext = 0;
			Buttons_[i]->Enabled      = false;
		}else{
			len = ToString(set[cur_rec_no]["項目1略称"]).Length();
			if (len <= 20)
			...
			Buttons_[i]->Caption     = ToString(set[cur_rec_no]["項目1略称"]);
			Buttons_[i]->Tag         = ToInt(set[cur_rec_no]["項目1ID"]);
...
//---------------------------------------------------------------------------
void __fastcall TItem1SelectForm::CancelButtonClick(TObject *Sender)
{
	SFM.Transit(PrevState(SFM.GetCurrentStateID()));
}
//---------------------------------------------------------------------------
void __fastcall TItem1SelectForm::Item1ButtonClick(TObject *Sender)
{
	SFM.stash["項目1ID"] = dynamic_cast<TButton *>(Sender)->Tag;
	SFM.stash["項目1略称"]   = dynamic_cast<TButton *>(Sender)->Caption;

	SFM.Transit(NextState(SFM.GetCurrentStateID()));
}
//---------------------------------------------------------------------------
void __fastcall TItem1SelectForm::ScrollUpButtonClick(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TItem1SelectForm::ScrollDownButtonClick(TObject *Sender)
{
	int lefttop = lefttop_rec_no_;
	lefttop += 3;
	AllocateRecordToButtons(lefttop);
...
//---------------------------------------------------------------------------
void __fastcall TItem1SelectForm::Kana1ButtonClick(TObject *Sender)
{
