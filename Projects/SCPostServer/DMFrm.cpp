...
bool __fastcall TDM::Init()
{
	...
	try{
		...
		Application->MessageBox(("データベース接続に失敗しました\r\n" + e.Message).c_str(),
			Application->Title.c_str(), MB_OK | MB_ICONHAND | MB_TOPMOST);
		return false;
	}
...
//---------------------------------------------------------------------------
// 指定拠点の計量データのうち、該当車両による計量が何回目かを取得
int __fastcall TDM::GetNextKeiryoCount(int place_id, int date, int car_id)
{
	AnsiString sql;
	NsRecordSet set;

	sql = " SELECT COUNT(*) AS 回数 FROM D_計量"
		  " WHERE "
          "     計量日 = " + IntToStr(date) +
          " AND"
		  " 	拠点ID = " + IntToStr(place_id) +
	...
	if (!DBI.GetRecordSet(sql, set))
		return 1;
	return ToInt(SET_TOP(set)["回数"]);
}
//---------------------------------------------------------------------------
// 指定車両の滞留件数を取得（トベ用）
int __fastcall TDM::GetRemainedCount(int place_id, int car_id)
{
	AnsiString sql;
	NsRecordSet set;

	sql = " SELECT COUNT(計量ID) FROM D_計量"
		  " WHERE "
		  " 	拠点ID = " + IntToStr(place_id) +
		  " AND"
		  " 	完了区分 = 0"
