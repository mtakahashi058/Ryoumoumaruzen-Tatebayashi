...
void __fastcall TMainForm::OutputActionExecute(TObject *Sender)
{
	...
	XlBook book(ExtractFilePath(Application->ExeName) + "順位表雛型.xls");
	try{
		book.open(true);
		...
		if (OrderComboBox->ItemIndex == 0){	// 金額順
			cells[3][4] = "重量 【kg】";
			cells[3][5] = "金額 【円】";
		}else{	// 数量順
			cells[3][4] = "金額 【円】";
		...
		// 総合計
		NsRecordSet set;
		if (DM->DBI.GetRecordSet("SELECT SUM(" + orderby + ") AS 合計 FROM V_計量 WHERE 計量日 BETWEEN " + IntToStr(DateToInt(FromDatePicker->Date)) + " AND " + IntToStr(DateToInt(ToDatePicker->Date)) + where, set))
		...
		while (!q_shukei->Eof){
			...
			if (OrderComboBox->ItemIndex == 0){	// 金額順
				cells[row][4] = q_shukei->FieldByName("正味重量")->AsCurrency;
				cells[row][5] = q_shukei->FieldByName("金額")->AsCurrency;
			}else{	// 数量順
				cells[row][4] = q_shukei->FieldByName("金額")->AsCurrency;
				cells[row][5] = q_shukei->FieldByName("正味重量")->AsCurrency;
			}
			row++;
			q_shukei->Next();
		}
...
bool __fastcall TMainForm::SetQuery()
{
	...
	where_rep += "取引区分：" + TorihikiComboBox->Items->Strings[TorihikiComboBox->ItemIndex] + "  ";
