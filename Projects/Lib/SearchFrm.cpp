...
void __fastcall TSearchForm::btnSearchClick(TObject *Sender)
{
	...
	if (cedtSearch->Text == ""){
		...
		if (based_sql.Pos("WHERE") == 0)
			Query->SQL->Text = based_sql + " WHERE " + DM->DBI.FullSearchSQL(cedtSearch->Text, table_name.c_str(), list_, true) + orderby_sql;
		else
			Query->SQL->Text = based_sql + " AND " + DM->DBI.FullSearchSQL(cedtSearch->Text, table_name.c_str(), list_, true) + orderby_sql;
	}
