...
#pragma resource "*.dfm"
TQRInnerChitForm *QRInnerChitForm;
//---------------------------------------------------------------------------
__fastcall TQRInnerChitForm::TQRInnerChitForm(TComponent* Owner)
    : TForm(Owner)
{
}
//---------------------------------------------------------------------------
void __fastcall TQRInnerChitForm::Print(int keiryo_id)
{
	...
	QR->Page->LeftMargin = ToCurrency(DM->INI["Chit::InnerPrinterLeft"]);
	QR->Page->TopMargin  = ToCurrency(DM->INI["Chit::InnerPrinterTop"]);
	...
	SetPrinter(QR, ToString(DM->INI["Chit::InnerPrinter"]));
	int copies = ToInt(DM->INI["Chit::InnerPrintCopy"]);
...
//---------------------------------------------------------------------------
void __fastcall TQRInnerChitForm::ChitQueryCalcFields(TDataSet *DataSet)
{
    int w1, w2;
