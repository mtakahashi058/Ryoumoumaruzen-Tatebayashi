...
//
//  void __fastcall TCADHVThread::UpdateCaption()
//  {
...
//
//---------------------------------------------------------------------------
__fastcall TCADHVThread::TCADHVThread(bool CreateSuspended, const WICommInfo& info)
    : TThread(CreateSuspended), info_(info)
{
}
//---------------------------------------------------------------------------
bool __fastcall TCADHVThread::formatMessage(std::string& msg)
{
    LPVOID buff;
...
//---------------------------------------------------------------------------
void __fastcall TCADHVThread::Execute()
{
    ...
    while (!Terminated){
		...
		else if (buff[0] == 'U' && buff[1] == 'S')
			weight_.status = WIWeight::wiUnstable;
		else if (buff[0] == 'O' && buff[1] == 'L')
