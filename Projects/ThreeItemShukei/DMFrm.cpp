										...
										AnsiString where)
{
	AnsiString sql =
		...
		" 	V.$TARGETI3$ AS 項目ID3, MAX(V.$TARGETC3$) AS 項目コード3, MAX(V.$TARGETN3$) AS 項目略称3,"
		"  	MAX(KEN.回数) AS 回数,"
		"  	SUM(V.正味重量) AS 合計重量,"
		"  	SUM(V.数量) AS 合計数量,"
		"  	V.単位コード, MAX(V.単位マスタ名称) AS 単位,"
		"  	SUM(V.金額) AS 合計金額, "
		"  	RIGHT('000000000'+ CAST(MAX(V.$TARGETI1$) AS VARCHAR), 9) + RIGHT('000000000'+ CAST(MAX(V.$TARGETI2$) AS VARCHAR), 9) AS 切替ID"
		" FROM"
		" 	V_計量 AS V"
		"  	LEFT OUTER JOIN"
		"  		("
		...
		" 			("
		" 			SELECT"
		" 				V.$TARGETI1$, V.$TARGETI2$, V.$TARGETI3$, V.単位コード, V.計量日, V.伝票No"
		" 			FROM"
		" 				V_計量 AS V"
		" 			WHERE"
		" 				$WHERE$"
		...
		" 			) AS X"
		" 		GROUP BY"
		" 			$TARGETI1$, $TARGETI2$, $TARGETI3$, 単位コード"
		" 		) AS KEN"
		"  	ON ISNULL(V.$TARGETI1$, 0) = ISNULL(KEN.$TARGETI1$, 0) AND ISNULL(V.$TARGETI2$, 0) = ISNULL(KEN.$TARGETI2$, 0) AND ISNULL(V.$TARGETI3$, 0) = ISNULL(KEN.$TARGETI3$, 0) AND ISNULL(V.単位コード, 0) = ISNULL(KEN.単位コード, 0)"
		" WHERE"
		" 	$WHERE$"
		" AND"
		" 	V.完了区分 = 1"
		" GROUP BY"
		"  	V.$TARGETI1$, V.$TARGETI2$, V.$TARGETI3$, V.単位コード"
		" ORDER BY"
		"  	項目コード1, 項目ID1, 項目コード2, 項目ID2, 項目コード3, 項目ID3, V.単位コード";
