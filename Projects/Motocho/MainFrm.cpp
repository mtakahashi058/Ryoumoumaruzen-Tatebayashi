...
void __fastcall TMainForm::OutputActionExecute(TObject *Sender)
{
	...
	//Table2Str(QRForm->Query, records.get(), true); // 2018/06/27 "use_displaytext = true"を追加
	SaveDialog->FileName = "計量元帳_" + Date().FormatString("yyyymmdd") + ".csv";
...
//		QRForm->HinmokuCodeLabel->Enabled = true;
	...
	QRForm->IoLabel->Enabled = ToInt(DM->config_["入出庫使用区分"]) == 1 ? true : false;
	QRForm->IoText->Enabled  = ToInt(DM->config_["入出庫使用区分"]) == 1 ? true : false;

	if (ToInt(DM->config_["重量引き使用区分"]) == 1){
		QRForm->JuryobikiLabel->Enabled = true;
		QRForm->JuryobikiText->Enabled  = true;

		buff = ToString(DM->config_["重量引き名称"]);
		buff = AnsiReplaceStr(buff, " ", "");
		buff = AnsiReplaceStr(buff, "　", "");
	...
	QRForm->DetailBikoLabel->Enabled = ToInt(DM->config_["単品多段区分"]) == 1 ? false : true;
	QRForm->DetailBikoText->Enabled  = ToInt(DM->config_["単品多段区分"]) == 1 ? false : true;
	QRForm->LineNoLabel->Enabled     = ToInt(DM->config_["単品多段区分"]) == 1 ? false : true;
	QRForm->LineNoText->Enabled      = ToInt(DM->config_["単品多段区分"]) == 1 ? false : true;
}
//---------------------------------------------------------------------------
...
{
	HinbunNameEdit->Text = ToString(DM->DBI.Lookup("M_品目分類", "品目分類コード", HinbunCodeEdit->Text.ToIntDef(0), "品目分類名称"));
}
//---------------------------------------------------------------------------
...
void __fastcall TMainForm::HinbunSearchButtonClick(TObject *Sender)
{
	...
	SearchForm->based_sql = "SELECT 品目分類コード, 品目分類名称 FROM M_品目分類 ";
	SearchForm->orderby_sql = " ORDER BY 品目分類コード ";
	SearchForm->table_name = "M_品目分類";
	SearchForm->list_->CommaText = "品目分類コード,品目分類名称";
	if (SearchForm->ShowModal() != mrOk)
		return;
	HinbunCodeEdit->Text = SearchForm->Query->FieldByName("品目分類コード")->AsInteger;
	HinbunCodeEdit->SetFocus();
}
//---------------------------------------------------------------------------
