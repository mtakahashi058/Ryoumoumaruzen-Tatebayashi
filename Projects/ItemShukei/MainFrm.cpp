...
*/
	QRForm->TitleLabel->Caption = SelectItemComboBox->Items->Strings[SelectItemComboBox->ItemIndex] + "別集計表";
...
void __fastcall TMainForm::ExcelOut()
{
	...
	XlBook book(ExtractFilePath(Application->ExeName) + "項目別集計表.xls");
	try{
		...
		// タイトル・日付など埋め込み
		cells[1][1] = SelectItemComboBox->Items->Strings[SelectItemComboBox->ItemIndex] + "別集計表";
		...
		while (!query->Eof){
			...
			cells[row][3] = FormatFloat("#,##0.#", query->FieldByName("合計重量")->AsCurrency);
		...
		cells[row][3] = "=SUM(C5:C" + IntToStr(row - 1) + ")";
		cells[row][6] = "=SUM(F5:F" + IntToStr(row - 1) + ")";
