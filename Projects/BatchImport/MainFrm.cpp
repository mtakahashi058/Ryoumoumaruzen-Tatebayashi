...
void __fastcall TMainForm::FormShow(TObject *Sender)
{
    ...
    std_unit_code_ = ToInt(DM->CFG["標準単位"]);
    StdUnitLabel->Caption = "標準単位は「" + ToString(DM->DBI.Lookup("M_単位", "単位コード", std_unit_code_, "単位名称")) + "」";
}
//---------------------------------------------------------------------------
...
bool __fastcall TMainForm::PostRecord(AnsiString record)
{
    ...
    TDateTime now(Now());
    int cur_year = now.FormatString("yyyy").ToInt();
    int cur_mmdd = now.FormatString("mmdd").ToInt();
    ...
    int car_no     = buff.SubString(20, 8).Trim().SubString(1, 6).ToIntDef(0);

    buff = AnsiReplaceStr(record, " ", "0");

    int no         = buff.SubString(10, 4).ToIntDef(0);
    int mmdd       = buff.SubString(14, 4).ToIntDef(0);
    int item1_code = buff.SubString(29, 3).ToIntDef(0);
    int item2_code = buff.SubString(33, 3).ToIntDef(0);
    int item3_code = buff.SubString(37, 3).ToIntDef(0);
    int item4_code = buff.SubString(41, 3).ToIntDef(0);
    int weight1    = buff.SubString(44, 8).ToIntDef(0);
    int hhnn1      = buff.SubString(52, 4).ToIntDef(0);
    int weight2    = buff.SubString(56, 8).ToIntDef(0);
    int hhnn2      = buff.SubString(64, 4).ToIntDef(0);
    int net        = buff.SubString(97, 7).ToIntDef(0);

    int year = mmdd > cur_mmdd ? cur_year - 1 : cur_year;   // 今日よりおっきい日付の場合去年
    int date  = year * 10000 + mmdd;
    ...
    *(scm_codes[cnv_table[4]]) = item4_code;
    ...
    ////////////////////////////////////////////////////////////////////////////
    // 計量日時の取得
    TDateTime time1 = EncodeDateTime(year, mmdd / 100, mmdd % 100, hhnn1 / 100, hhnn1 % 100, 0, 0);
    TDateTime time2 = EncodeDateTime(year, mmdd / 100, mmdd % 100, hhnn2 / 100, hhnn2 % 100, 0, 0);
...
*/
    int tani_id, tani_code;
    Currency net_ton;

    if (!ConvertCheckBox->Checked){
        tani_code = std_unit_code_;
        tani_id   = ToInt(DM->DBI.Lookup("M_単位", "単位コード", tani_code, "単位ID"));

        if (tani_code == 3) // kgの場合
            net_ton = Currency(net);
        else                // それ以外はtベースにする
            net_ton = Currency(net) / Currency(1000);
    }else{
        tani_id   = DM->GetKanzanID(scm_toku_id, scm_meig_id);
        tani_code = DM->DBI.Lookup("M_単位", "単位ID", tani_id, "単位コード");
	    net_ton   = Currency(net) / Currency(1000);
    }
