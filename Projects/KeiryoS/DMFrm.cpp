...
//---------------------------------------------------------------------------
// 単価の取得 (組合せによって取得されない単価があるため修正 2017/05/11)
Currency __fastcall TDM::GetTanka(int toku_id, int meig_id, int item1_id, int item2_id, int tani_id)
{
	AnsiString sql;
	NsRecordSet set;

	// 「得意先○・品目○・項目１○・項目２○・単位○」5/5
	sql = " SELECT	* FROM M_単価"
		  " WHERE	得意先ID = " + IntToStr(toku_id) +
		  " AND 	品目ID   = " + IntToStr(meig_id) +
		  " AND 	項目1ID  = " + IntToStr(item1_id) +
		  " AND 	項目2ID  = " + IntToStr(item2_id) +
		  " AND 	単位ID   = " + IntToStr(tani_id);
	if (DM->DBI.GetRecordSet(sql, set))
		return ToCurrency(SET_TOP(set)["単価"]);

	// 「得意先○・品目○・項目１○・項目２×・単位○」4/5
	sql = " SELECT * FROM M_単価"
		  " WHERE 	得意先ID  = " + IntToStr(toku_id) +
		  " AND 	品目ID    = " + IntToStr(meig_id) +
		  " AND 	項目1ID   = " + IntToStr(item1_id) +
		  " AND 	(項目2ID  = 0 OR 項目2ID IS NULL)"
		  " AND 	単位ID    = " + IntToStr(tani_id);
	if (DM->DBI.GetRecordSet(sql, set))
		return ToCurrency(SET_TOP(set)["単価"]);
	...
	// 「得意先×・品目○・項目１○・項目２○・単位○」4/5
	sql = " SELECT	* FROM M_単価"
		  " WHERE 	(得意先ID = 0 OR 得意先ID IS NULL)"
		  " AND 	品目ID   = " + IntToStr(meig_id) +
		  " AND 	項目1ID  = " + IntToStr(item1_id) +
		  " AND 	項目2ID  = " + IntToStr(item2_id) +
		  " AND 	単位ID   = " + IntToStr(tani_id);
	...
	// 「得意先×・品目×・項目１×・項目２×・単位○」1/5
	sql = " SELECT * FROM M_単価"
		  " WHERE 	(得意先ID = 0 OR 得意先ID IS NULL)"
		  " AND 	(品目ID   = 0 OR 品目ID IS NULL)"
		  " AND 	(項目1ID  = 0 OR 項目1ID IS NULL)"
		  " AND 	(項目2ID  = 0 OR 項目2ID IS NULL)"
		  " AND 	単位ID   = " + IntToStr(tani_id);
	if (DM->DBI.GetRecordSet(sql, set))
