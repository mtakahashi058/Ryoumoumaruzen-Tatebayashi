...
//	RichEdit1->Lines->Text = QRForm->Query->SQL->Text;
	...
	QRForm->TitleLabel->Caption    = SelectItem1ComboBox->Items->Strings[SelectItem1ComboBox->ItemIndex] + "別" +
									 ...
									 SelectItem3ComboBox->Items->Strings[SelectItem3ComboBox->ItemIndex] + "別集計表【取引】";
...
void __fastcall TMainForm::ExcelOut()
{
	...
	XlBook book(ExtractFilePath(Application->ExeName) + "取引3項目別集計表.xls");
	try{
		...
		// タイトル・日付など埋め込み
		cells[1][1] = SelectItem1ComboBox->Items->Strings[SelectItem1ComboBox->ItemIndex] + "別" + SelectItem2ComboBox->Items->Strings[SelectItem2ComboBox->ItemIndex] + "別" + SelectItem3ComboBox->Items->Strings[SelectItem3ComboBox->ItemIndex] + "別集計表【取引】";
		...
		for (int i = 0; i < row_list->Count; i++){
			if (expr1 != "")
				...
				expr1 += "=";
			expr1 += "D" + row_list->Strings[i];

			if (expr3 != "")
				expr3 += " + ";
			else
				expr3 += "=";
			expr3 += "G" + row_list->Strings[i];
		}
		cells[row][4] = expr1;
		cells[row][7] = expr3;
