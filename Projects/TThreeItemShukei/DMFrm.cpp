										...
										AnsiString where)
{
	AnsiString sql =
		...
		"  	SUM(V.金額) AS 合計金額, "
		" 	RIGHT('000000000'+ CAST(V.$TARGETI1$ AS VARCHAR), 9) + RIGHT('000000000'+ CAST(V.$TARGETI2$ AS VARCHAR), 9) AS 切替ID"
		" FROM"
		" 	V_取引 AS V"
		"  	LEFT OUTER JOIN"
		"  		("
		...
		" 			("
		" 			SELECT"
		" 				V.$TARGETI1$, V.$TARGETI2$, V.$TARGETI3$, V.単位コード, V.取引日, V.伝票No"
		" 			FROM"
		" 				V_取引 AS V"
		" 			WHERE"
		" 				$WHERE$"
