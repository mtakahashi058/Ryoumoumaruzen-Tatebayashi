...
void __fastcall TMainForm::OutputActionExecute(TObject *Sender)
{
	...
	XlBook book(ExtractFilePath(Application->ExeName) + "取引月別集計表雛型.xls");
	try{
		book.open(true);
		...
		cells[2][4] = QRForm->RangeLabel->Caption;
		...
		for (int i = 0; i < 12; i++){
			...
			if (q_shukei->Locate("年月", month, TLocateOptions())){
				cells[5 + i][2] = q_shukei->FieldByName("金額")->AsCurrency;
			}else{
				cells[5 + i][2] = "0";
			}
...
bool __fastcall TMainForm::SetQuery()
{
	...
	if (IoDivComboBox->ItemIndex > 0)
		QRForm->Query->SQL->Strings[19] = "AND	取引区分 = " + IntToStr(IoDivComboBox->ItemIndex);
	else
		QRForm->Query->SQL->Strings[19] = AnsiString();
	QRForm->RangeLabel->Caption = QRForm->RangeLabel->Caption + "  取引区分：" + IoDivComboBox->Items->Strings[IoDivComboBox->ItemIndex];

	if (GenkakeComboBox->ItemIndex != 0){
		QRForm->Query->SQL->Strings[20] = "AND	現掛区分 = " + IntToStr(GenkakeComboBox->ItemIndex);
