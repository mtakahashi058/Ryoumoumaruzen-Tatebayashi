...
void __fastcall TDisplayForm::ShowRecord(int id)
{
	...
	//Show();
    // default win7 taskbar: about 40 / notify form 35
    Height = 690;
    //Height = 597;

    if (ToInt(SET_TOP(set)["完了区分"]) == 1){
        HeaderPanel->Caption = "カード計量が完了しました";
    }else{
        HeaderPanel->Caption = "１回目の計量が完了しました";
    }
}
//---------------------------------------------------------------------------
void __fastcall TDisplayForm::Clear()
{
    HeaderPanel->Caption = "待機中";
    ClearForm();
}
//---------------------------------------------------------------------------
