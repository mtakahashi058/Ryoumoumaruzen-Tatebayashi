...
private:	// ユーザー宣言
	...
	bool on_weighing_[SCALE_COUNT]; 		// 計量中か否か
	bool reversed_[SCALE_COUNT];            // 逆挿入されたか
	bool on_error_[SCALE_COUNT];            // 計量エラー発生中か否か
	bool on_printing_[SCALE_COUNT];         // 印刷中かどうか
	bool limit_over_[SCALE_COUNT];          // 過積載かどうか
	bool on_wait_exit_[SCALE_COUNT];        // 退車待ちかどうか
	...
	AD4385StartInfo ad85_info_[SCALE_COUNT];    // AD4385 初期設定
    int top_margin_[SCALE_COUNT];
	int display_weight_[SCALE_COUNT];			// 表示重量
	unsigned int thread_id_[SCALE_COUNT];		// スレッドID
	AD4385Warning ad85_warning_[SCALE_COUNT];	// AD4385 からの警告情報
	AD4385Status ad85_status_[SCALE_COUNT];		// AD4385 からのステータス情報
	AD4385Params ad85_params_[SCALE_COUNT];     // AD4385 への指示
