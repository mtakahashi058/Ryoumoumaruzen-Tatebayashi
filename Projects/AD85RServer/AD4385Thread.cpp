...
bool __fastcall TAD4385Thread::receiveUntilEot()
{
    ...
    while (1){
        ...
        if (s == esEos){
            ...
            // カード＆キー情報を構造体にセット
            card_key_info_.card.id    = ackl::fromString<int>(buff.substr(3, 6));
            card_key_info_.card.item1 = ackl::fromString<int>(buff.substr(9, 3));
            card_key_info_.card.item2 = ackl::fromString<int>(buff.substr(12, 3));
            card_key_info_.card.item3 = ackl::fromString<int>(buff.substr(15, 3));
            card_key_info_.card.item4 = ackl::fromString<int>(buff.substr(18, 3));
            ...
            required_info_.item_code = ackl::fromString<int>(buff.substr(3, 3));
        }
    }
    return true;
}
        ...
        while (PeekMessage(&msg, 0, UM_AD4385_COMMAND, UM_AD4385_COMMAND, PM_REMOVE)){
            ...
            try{
                ...
                case cmPrint:
                    {
                        if (top_margin_)
                            if (!ad4385.setTopMargin(top_margin_))
...
{
    AD4385RPostForm->OnAD4385Warning(start_info_.tag);
}
//---------------------------------------------------------------------------
void __fastcall TAD4385Thread::CallAD4385EventHandler()
{
    AD4385RPostForm->OnAD4385Event(start_info_.tag);
}
//---------------------------------------------------------------------------
