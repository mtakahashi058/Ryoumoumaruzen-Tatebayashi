...
void __fastcall TMainForm::ExcelOut()
{
	...
	Currency sum_net1, sum_net2, sum_net_total;
	...
	XlBook book(ExtractFilePath(Application->ExeName) + "2項目別明細表.xls");
	try{
		...
		row = 5;
		sum_net1 = sum_net2 = sum_net_total = 0;
		...
		while (!query->Eof){
			...
			if (ToInt(DM->config_["単価使用区分"]) == 1){
				...
				cells[row][10] = FormatFloat("#,##0.0##", query->FieldByName("単価")->AsCurrency);
			...
			if (query->Eof){
				...
				if (SumHinmokuCheckBox->Checked){
					...
					while(!QRForm->MeigaraQuery->Eof){
						...
						line_cells = "B" + IntToStr(row) + ":K" + IntToStr(row);
						exBorders = sheet.Exec(PropertyGet("Range") << line_cells).Exec(PropertyGet("Borders") << 9);
						exBorders.Exec(PropertySet("LineStyle") << 2);
