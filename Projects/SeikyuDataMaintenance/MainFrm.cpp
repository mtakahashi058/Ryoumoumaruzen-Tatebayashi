...
#pragma resource "*.dfm"
TMainForm *MainForm;
AnsiString GRID_CONFIG = "請求DMグリッド配置.txt";
//---------------------------------------------------------------------------
__fastcall TMainForm::TMainForm(TComponent* Owner)
	: TForm(Owner), grid_cleared_(false)
{
...
void __fastcall TMainForm::FormShow(TObject *Sender)
{
	...
	SetPrinter(QRSeikyuItiranForm->QuickRep, ToString(DM->INI["Printer::Laser"]));
...
{
	if (DetailForm->ShowModalWithID(DM->Q_Itiran->FieldByName("請求ID")->AsInteger) != mrOk)
		return;
	ApplyButton->OnClick(NULL);
}
//---------------------------------------------------------------------------
void __fastcall TMainForm::Action3Execute(TObject *Sender)
{
	...
	try{
		...
		NsFieldSet keys;
		keys << NsField("請求ID", DM->Q_Itiran->FieldByName("請求ID")->AsInteger);

		DM->DBI.Delete("D_請求", keys);
...
void __fastcall TMainForm::Action5Execute(TObject *Sender)
{
	...
	try{
		...
		while (std::getline(ifs, buff)){
			...
			sql = "SELECT 請求ID FROM V_請求 WHERE 締切日 = " + records->Strings[0] + " AND 得意先コード = " + records->Strings[1];
			set.clear();
			...
			fields << NsField("[締切日]",       records->Strings[0].ToIntDef(0))
				   ...
				   << NsField("[前回請求額]",   StrToCurrDef(records->Strings[2], 0))
				   << NsField("[今回入金額]",   StrToCurrDef(records->Strings[3], 0))
				   << NsField("[今回調整額]",   StrToCurrDef(records->Strings[4], 0))
				   << NsField("[今回繰越額]",   StrToCurrDef(records->Strings[5], 0))
				   << NsField("[今回売上額]",   StrToCurrDef(records->Strings[6], 0))
				   << NsField("[今回消費税額]", StrToCurrDef(records->Strings[7], 0))
				   << NsField("[今回請求額]",   StrToCurrDef(records->Strings[8], 0))
				   << NsField("[入金予定日]",   records->Strings[9].ToIntDef(0))
				   << NsField("[更新日時]",     Now());
			...
			if (!id){
				...
				DM->DBI.Update("D_請求", "請求ID", id, fields);
				u_count++;
			}
		}
...
void __fastcall TMainForm::Action9Execute(TObject *Sender)
{
	...
	QRSeikyuItiranForm->QuickRep->Print();
}
//---------------------------------------------------------------------------
void __fastcall TMainForm::Action10Execute(TObject *Sender)
{
	...
	QRSeikyuItiranForm->QuickRep->Preview();
}
//---------------------------------------------------------------------------
void __fastcall TMainForm::Action11Execute(TObject *Sender)
{
	...
	records->Append("請求一覧,出力範囲：" + GetSeikyuRange());

	Table2Str(query, records.get(), true);
	SaveDialog->FileName = "請求一覧表_" + Date().FormatString("yyyymmdd") + ".csv";
...
void __fastcall TMainForm::ApplyButtonClick(TObject *Sender)
{
	...
	if (!ViewZeroCheckBox->Checked){
		if (where != "")
			where += " AND";
		where += " (前回請求額 <> 0 OR 今回入金額 <> 0 OR 今回売上額 <> 0)";
	}
	...
	QRSeikyuItiranForm->lblRange->Caption = GetSeikyuRange();
...
//---------------------------------------------------------------------------
// 請求一覧表の出力範囲を返す
AnsiString __fastcall TMainForm::GetSeikyuRange()
{
	AnsiString asRange = "";
	if (SimebiEdit->Text != "")
