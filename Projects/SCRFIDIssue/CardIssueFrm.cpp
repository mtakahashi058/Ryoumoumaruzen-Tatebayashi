...
void __fastcall TCardIssueForm::FormShow(TObject *Sender)
{
    ...
    if (!DM->cardService.Establish()){
        ...
        Application->MessageBox(AnsiString("SmartCardとの通信確立に失敗しました - " + err_text).c_str(), Caption.c_str(),  MB_OK | MB_ICONSTOP);
        PostMessage(Handle, WM_CLOSE, 0, 0);
        return;
    }
    ...
    if (readers.empty()){
        Application->MessageBox("SmartCardリーダが存在しません", Caption.c_str(), MB_OK | MB_ICONSTOP);
        PostMessage(Handle, WM_CLOSE, 0, 0);
        return;
    }
...
void __fastcall TCardIssueForm::IssueButtonClick(TObject *Sender)
{
	...
	try{
        ...
        const toku_id  = ToInt(DM->DBI.Lookup("M_得意先", "得意先コード", TokuEdit->Text.ToIntDef(0), "得意先ID"));
        const item1_id = ToInt(DM->DBI.Lookup("M_項目1",  "項目1コード",  Item1Edit->Text.ToIntDef(0), "項目1ID"));
        const item2_id = ToInt(DM->DBI.Lookup("M_項目2",  "項目2コード",  Item2Edit->Text.ToIntDef(0), "項目2ID"));
        const hin_id   = ToInt(DM->DBI.Lookup("M_品目",   "品目コード",   HinEdit->Text.ToIntDef(0),   "品目ID"));
