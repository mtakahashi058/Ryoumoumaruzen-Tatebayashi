...
void __fastcall TSearchForm::cedtSearchEnter(TObject *Sender)
{
	HIMC imc = ImmGetContext(dynamic_cast<TWinControl *>(Sender)->Handle);
    if (imc){
    	if (!ImmGetOpenStatus(imc)){
			keybd_event(VK_KANJI, 0, 0, 0);
			keybd_event(VK_KANJI, 0, KEYEVENTF_KEYUP, 0);
		}
    	ImmReleaseContext(dynamic_cast<TWinControl *>(Sender)->Handle, imc);
    }
}
//---------------------------------------------------------------------------
