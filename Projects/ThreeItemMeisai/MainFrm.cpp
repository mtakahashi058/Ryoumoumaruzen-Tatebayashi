...
bool __fastcall TMainForm::OpenQuery()
{
	...
	AnsiString where = " 計量日 BETWEEN " + FromDatePicker->Date.FormatString("yyyymmdd") + " AND " + ToDatePicker->Date.FormatString("yyyymmdd");
...
void __fastcall TMainForm::ExcelOut()
{
	...
	AnsiString search_target = dir + "3項目別明細表" + Date().FormatString("yyyymmdd") + "*.xls";
	HANDLE find_handle;
	...
	XlBook book(ExtractFilePath(Application->ExeName) + "3項目別明細表.xls");
	...
	try{
		...
		// タイトル・日付など埋め込み
		cells[1][1] = SelectItem1ComboBox->Items->Strings[SelectItem1ComboBox->ItemIndex] + "別" + SelectItem2ComboBox->Items->Strings[SelectItem2ComboBox->ItemIndex] + "別" + SelectItem3ComboBox->Items->Strings[SelectItem3ComboBox->ItemIndex] + "別明細表";
		...
		while (!query->Eof){
			...
			if (old_item2 != query->FieldByName("項目コード2")->AsInteger){
				...
				cells[row][3]  = IntToDate(query->FieldByName("計量日")->AsInteger);
			}else if (old_item3 != query->FieldByName("項目コード3")->AsInteger){
				cells[row][2]  = query->FieldByName("項目コード3")->AsString + "：" + query->FieldByName("項目略称3")->AsString;
				cells[row][3]  = IntToDate(query->FieldByName("計量日")->AsInteger);
			}else if (old_date != query->FieldByName("計量日")->AsInteger){
				cells[row][3]  = IntToDate(query->FieldByName("計量日")->AsInteger);
			}
			...
			cells[row][7]  = FormatFloat("#,##0.#", query->FieldByName("正味重量")->AsCurrency);
			if (ToInt(DM->config_["単価使用区分"]) == 1){
				cells[row][8]  = FormatFloat("#,##0.0##", query->FieldByName("数量")->AsCurrency);
				cells[row][9]  = query->FieldByName("単位マスタ名称")->AsString;
				cells[row][10] = FormatFloat("#,##0.###", query->FieldByName("単価")->AsCurrency);
				cells[row][11] = FormatFloat("#,##0", query->FieldByName("金額")->AsCurrency);
			}
			sum_net1      += query->FieldByName("正味重量")->AsCurrency;
			sum_net2      += query->FieldByName("正味重量")->AsCurrency;
			sum_net3      += query->FieldByName("正味重量")->AsCurrency;
			sum_net_total += query->FieldByName("正味重量")->AsCurrency;
			...
			old_date  = query->FieldByName("計量日")->AsInteger;
			...
			if (query->Eof){
				...
				cells[row][7]  = sum_net3;
				if (ToInt(DM->config_["単価使用区分"]) == 1)
					cells[row][11] = sum_money3;
				sum_net3   = 0;
				sum_money3 = 0;
				row++;
				...
				if (old_item2 != query->FieldByName("項目コード2")->AsInteger){
					...
					cells[row][7]  = sum_net2;
					if (ToInt(DM->config_["単価使用区分"]) == 1)
						cells[row][11] = sum_money2;
					sum_net2   = 0;
					sum_money2 = 0;
					row++;
				}
			...
			if (query->Eof || old_item1 != query->FieldByName("項目コード1")->AsInteger){
				...
				cells[row][7]  = sum_net1;
				if (ToInt(DM->config_["単価使用区分"]) == 1)
					cells[row][11] = sum_money1;
				sum_net1   = 0;
				sum_money1 = 0;
				row++;
				...
				if (query->Eof){
					if (SumHinmokuCheckBox->Checked){
						...
						while(!QRForm->MeigaraQuery->Eof){
							...
							cells[row][7]  = QRForm->MeigaraQuery->FieldByName("重量")->AsCurrency;
							if (ToInt(DM->config_["単価使用区分"]) == 1)
								cells[row][11] = QRForm->MeigaraQuery->FieldByName("金額")->AsCurrency;
					...
					row++;
					cells[row][2]  = "【 合    計 】";
					cells[row][7]  = sum_net_total;
					if (ToInt(DM->config_["単価使用区分"]) == 1)
						cells[row][11] = sum_money_total;
					sum_net_total   = 0;
					sum_money_total = 0;
				}

				if (sheet_count == max_sheet){
					book.getAppObj().Exec(PropertyGet("ActiveWorkBook")).Exec(Procedure("SaveAs") << dir + "3項目別明細表" + Date().FormatString("yyyymmdd") + "_" +  IntToStr(book_count) + ".xls");
					book.quit();
				...
				for (int i = 0; i < detail_rows->Count; i++){
					line_cells = "A" + detail_rows->Strings[i] + ":K" + detail_rows->Strings[i];
				...
				for (int i = 0; i < meisum_rows->Count; i++){
					line_cells = "B" + meisum_rows->Strings[i] + ":K" + meisum_rows->Strings[i];
					exBorders = sheet.Exec(PropertyGet("Range") << line_cells).Exec(PropertyGet("Borders") << 9);
					exBorders.Exec(PropertySet("LineStyle") << 2);
				}

				if (query->Eof){
					line_cells = "A" + IntToStr(row) + ":K" + IntToStr(row);
					exBorders = sheet.Exec(PropertyGet("Range") << line_cells).Exec(PropertyGet("Borders") << 9);
