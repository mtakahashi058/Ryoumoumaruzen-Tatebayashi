...
//---------------------------------------------------------------------------
void __fastcall TDisplayForm::ShowRecord(int id, AnsiString target_scale, int scale_position)
{
    // 画面の全体の幅を取得します
    int display_width = Screen->Width;

    // 3分割したフォームの幅を計算します
    int segment_width = display_width / 3;

    // left_positionの値に基づいてフォームの中心の位置を計算します
    int new_left = (scale_position - 1) * segment_width + segment_width/2 - DisplayForm->Width/2;

    if (new_left < 0) new_left = 0;

    TargetNameLabel->Caption = target_scale;
    DisplayForm->Position = poDesigned;
    DisplayForm->Left = new_left;
