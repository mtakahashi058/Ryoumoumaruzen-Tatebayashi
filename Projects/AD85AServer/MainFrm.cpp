...
void __fastcall TAD4385PostForm::OnAD4385CardInsert(int tag, const AD4385CardKeyInfo& info)
{
    ...
    if (card.str_id == ""){
        DM->Log("AD4385", "カード番号無効による計量中止（ID < 1）", 1);
    ...
    if (!DM->GetCardInfo(card.str_id.c_str(), card_rec)){
        ...
        DM->Log("AD4385", "未登録車両による計量中止（カード番号:" + str_temp + "）", 1);
    ...
    *(scm_codes[cnv_table[4]]) = card.item4;
    ...
    if (!remained_id && tare_w < 1){
        ...
        if (remained_id){
            ...
            code      = ToInt(SET_TOP(remained_set)["得意先ID"]);
            card_code = ToInt(card_rec["得意先ID"]);
            if (!code || (code != card_code)) {
                toku_id = ToInt(DM->DBI.Lookup("M_得意先", "得意先コード", toku_cd, "得意先ID"));
                DM->GetViewRec("得意先", toku_id, toku_rec);
            ...
            code = ToInt(SET_TOP(remained_set)["項目1ID"]);
            card_code = ToInt(card_rec["項目1ID"]);
            if (!code || (code != card_code)) {
                item1_id = ToInt(DM->DBI.Lookup("M_項目1", "項目1コード", item1_cd, "項目1ID"));
                DM->GetViewRec("項目1", item1_id, item1_rec);
            ...
            code = ToInt(SET_TOP(remained_set)["項目2ID"]);
            card_code = ToInt(card_rec["項目2ID"]);
            if (!code || (code != card_code)) {
                item2_id = ToInt(DM->DBI.Lookup("M_項目2", "項目2コード", item2_cd, "項目2ID"));
                DM->GetViewRec("項目2", item2_id, item2_rec);
            ...
            code = ToInt(SET_TOP(remained_set)["品目ID"]);
            card_code = ToInt(card_rec["品目ID"]);
            if (!code || (code != card_code)) {
                meig_id = ToInt(DM->DBI.Lookup("M_品目", "品目コード", meig_cd, "品目ID"));
                DM->GetViewRec("品目", meig_id, meig_rec);
    ...
    if (remained_id){
        ...
        DM->Log("AD4385", "計量完了（車番:" + IntToStr(car_no) + "）", 0);
        ...
        if (tare_w < 1){
            DM->Log("AD4385", "一回目計量完了（車番:" + IntToStr(car_no) + "）", 0);
        }else{
            // 一回計量により計量完了
            DM->Log("AD4385", "一回計量完了（車番:" + IntToStr(car_no) + "）", 0);
    ...
    //////////
        ...
        AnsiString str_id = card.str_id.c_str();
        DM->Log("AD4385", AnsiString("登録時例外（カード番号:") + str_id + "）: " + e.Message, 2);
