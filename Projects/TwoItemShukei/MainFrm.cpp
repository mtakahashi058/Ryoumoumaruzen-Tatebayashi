...
*/
	QRForm->TitleLabel->Caption    = SelectItem1ComboBox->Items->Strings[SelectItem1ComboBox->ItemIndex] + "別" + SelectItem2ComboBox->Items->Strings[SelectItem2ComboBox->ItemIndex] + "別集計表";
...
void __fastcall TMainForm::ExcelOut()
{
	...
	XlBook book(ExtractFilePath(Application->ExeName) + "2項目別集計表.xls");
	try{
		...
		while (!query->Eof){
			...
			cells[row][4] = FormatFloat("#,##0.#", query->FieldByName("合計重量")->AsCurrency);
			cells[row][5] = FormatFloat("#,##0.0##", query->FieldByName("合計数量")->AsCurrency);
			cells[row][6] = query->FieldByName("単位")->AsString;
			cells[row][7] = FormatFloat("#,##0", query->FieldByName("合計金額")->AsCurrency);
			...
			if (query->Eof || old_item1_id != query->FieldByName("項目ID1")->AsInteger){
				...
				cells[row][7] = "=SUM(G" + IntToStr(start_row) + ":G" + IntToStr(row - 1) + ")";
		...
		// 合計
		cells[row][1] = "【 合 計 】";
		cells[row][3] = "=SUMIF(B5:B" + IntToStr(row - 1) + ", \"【 " + SelectItem1ComboBox->Items->Strings[SelectItem1ComboBox->ItemIndex] + "計 】\",  C5:C" + IntToStr(row - 1) + ")";
		...
		cells[row][7] = "=SUMIF(B5:B" + IntToStr(row - 1) + ", \"【 " + SelectItem1ComboBox->Items->Strings[SelectItem1ComboBox->ItemIndex] + "計 】\",  G5:G" + IntToStr(row - 1) + ")";
